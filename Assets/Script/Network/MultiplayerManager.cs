...
public class MultiplayerManager : MonoBehaviourPunCallbacks
{
    ...
    private IEnumerator PingLoop()
    {
        while (true)
        {
            yield return new WaitForSeconds(3f); // espera 2 seg
    ...
    {
        // Try to create a new room when joining a random room fails.
        //PhotonNetwork.CreateRoom("Room_" + Random.Range(1, 10), new RoomOptions { MaxPlayers = 4 });
        Debug.Log("CreateRoom called after JoinRandom failed; waiting to join...");
        RoomOptions options = new RoomOptions { MaxPlayers = 4 };
        PhotonNetwork.CreateRoom("Room " + Random.Range(1, 10), options);
        // Do not access PhotonNetwork.CurrentRoom here — it will be null until the room is joined.
